...
class WarrantySerialRegistration(models.Model):
    ...
    #         }
    #
    #     # Get the first record's order_id to compare with others
    #     first_order_id = selected_records[0].order_id.id if selected_records[0].order_id else False
    #
    #     # Check if all records have the same order_id
    #     for record in selected_records:
    #         current_order_id = record.order_id.id if record.order_id else False
    #         if current_order_id != first_order_id:
    #             return {
    #                 'type': 'ir.actions.client',
    #                 'tag': 'display_notification',
    #                 'params': {
